...
class Admin extends CI_Controller{
    ...
    <div class="form-group">
		...
		<select id="optCodeSubCatalogy" name="subCatalogy" class="optional overall classes" onchange="loadDatatable(this.value)">
			<?php									
				foreach ($data as $c){?>
	...
	?>
			...
			<td><?php echo preg_replace("/<img[^>]+\>/i", "", word_limiter($listBaiViet[$i]['Content'], 28));  ?> </td>
			<td>
				<div class="radio">
			...
			<form action ="<?php echo base_url()."admin/EditArticle/".$listBaiViet[$i]['ID']?>" method="post">
											...
											<img src='http://localhost/TFV_Project/public/images/pencil-icon.png' alt='image'/>
											</button>
											<a onclick="callDialogDelete(<?php echo $listBaiViet[$i]['ID'] ?>,'<?php echo $listBaiViet[$i]['CodeSubCatalogy']?>')"><img src='http://localhost/TFV_Project/public/images/green-cross-icon.png' alt='image'/></a>
										</form>
			</td>
			</tr>
			<?php
	...
	public function input(){
		if($this->is_logged_in()){
				...
				if(!$recordPriority){}else{
					$this->MBaiviet->updatePriority($recordPriority->ID);
				}
				}else{
				$data->Priority = 0;
				}
